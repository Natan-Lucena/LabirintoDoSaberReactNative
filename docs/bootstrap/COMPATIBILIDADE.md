...
| 20  | `npx expo run:android` (T-108, no repositório, emulador `Pixel_3a_API_34`/API 34) — 1ª e 2ª tentativas                                | 1      | **Falhou 2x** (A-21, causa inicial suspeita: paths longos)                                                                                                                                                                    |
| 21  | `npx expo run:android` (T-108) — 3ª tentativa, após `LongPathsEnabled=1`                                                              | 1      | **Falhou de novo** com o mesmo erro: confirmou que long paths não era a causa (A-21)                                                                                                                                          |
| 23  | Validação manual no emulador (T-108): login mock (`educadora.mock@labirinto.test` / `senha123`) → Início                              | —      | **Login e Início confirmados** com dados mockados reais (ex.: "Olá, Aline Ribeiro Souza!", sessões do dia). Navegação para a tela de sessão (tela 04) não pôde ser confirmada nesta rodada (A-22). O usuário reportou instabilidade no login mock durante a sessão e pediu para não repetir tentativas; validação manual ficou a cargo dele |
| 24  | `npx expo prebuild --platform android` + `npx expo run:android` (T-108) — 5ª tentativa, com `android.package`/`ios.bundleIdentifier` = `com.labirintodosaber.app` (decisão do usuário, substituindo o valor gerado automaticamente `com.zer0rx.labirintodosaber`) | 0      | **BUILD SUCCESSFUL em 6m 21s**; app instalado como `com.labirintodosaber.app`, Metro bundlou e o app abriu no emulador (`mCurrentFocus=com.labirintodosaber.app/.MainActivity`) |
| 22  | `npx expo run:android` (T-108) — 4ª tentativa, após instalar `cmake;4.1.2` via `sdkmanager` e apontar `cmake.dir` em `android/local.properties` | 0      | **BUILD SUCCESSFUL em 5m 9s**; app instalado e aberto no emulador, Metro (JS) rodando em `:8081`                                                                                                                              |
...
| A-21 | `npx expo run:android` (debug) falhou 3x de forma reproduzível, não relacionado ao A-08: `ninja: error: manifest 'build.ninja' still dirty after 100 tries` na build C++ do `react-native-worklets`, com avisos de CMake de `CMAKE_OBJECT_PATH_MAX` (paths de objeto com 203+ caracteres sob `node_modules/.pnpm/react-native-worklets@.../android/.cxx/...`). Causa raiz real, isolada só na 3ª tentativa: **não é limite de path do Windows.** `LongPathsEnabled=1` foi habilitado (era `0`) e o build falhou de novo, idêntico. A causa é o Ninja 1.10.2, empacotado junto do CMake 3.22.1 (padrão do SDK/AGP), que não suporta paths longos no Windows (suporte só a partir do Ninja 1.12) | Instalar uma versão recente do CMake pelo `sdkmanager` (traz Ninja ≥1.12 embutido) e apontar o Gradle para ela via `android/local.properties` (gerado, não commitado): `cmake.dir=C\:\\Users\\<user>\\AppData\\Local\\Android\\Sdk\\cmake\\<versão>`. Comandos: `"<SDK>/cmdline-tools/bin/sdkmanager.bat" --sdk_root="<SDK>" --list` para ver versões e `"<SDK>/cmdline-tools/bin/sdkmanager.bat" --sdk_root="<SDK>" "cmake;<versão>"` para instalar (usado `cmake;4.1.2`, a maior disponível em 2026-09-25). Antes de repetir o build: `gradlew --stop` e limpar `android/app/.cxx` e o `.cxx`/`build` de `react-native-worklets` em `node_modules/.pnpm`. Manter `LongPathsEnabled=1` mesmo assim (não prejudica e ajuda outras libs nativas). Como `local.properties` não é versionado, documentar este passo para quem repetir o bootstrap. **Atualização:** `android.package`/`ios.bundleIdentifier` definidos como `com.labirintodosaber.app` (decisão do usuário, T-108), substituindo o valor `com.zer0rx.labirintodosaber` que o `expo prebuild` gerou automaticamente a partir da conta EAS logada na primeira rodada — não usar esse padrão automático como identificador definitivo do app | T-108 |
| A-22 | `adb shell uiautomator dump` chamado repetidamente (em sucessão rápida, para inspecionar bounds de elementos) crashou o processo shell do uiautomator no emulador (`IllegalStateException: UiAutomationService ... already registered`) e deixou o emulador com toques intermitentes/tela presa por vários minutos depois. O app em si não gerou nenhuma `FATAL EXCEPTION`; os únicos erros do app no logcat foram `FrameTracker` (timeout de animação do teclado, benigno) | Não chamar `uiautomator dump` mais de uma vez em sequência sem aguardar o processo anterior encerrar; preferir apenas screenshots (`adb exec-out screencap`) para inspeção visual em vez de dumps de acessibilidade repetidos. Se o emulador travar após isso, `am force-stop` + relançar o app e aguardar ao menos 8–10s antes de interagir | T-108 e qualquer tarefa que use automação de UI no emulador |
